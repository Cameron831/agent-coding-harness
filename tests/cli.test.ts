...
test("release dry-run prints resolved pull request input without creating a GitHub client", async () => {
  const stdout: string[] = [];
  const stderr: string[] = [];
...
test("release live creation defaults base to main and resolves current branch for head", async () => {
  const client = new FakeGitHubClient([], [
  ...
  const exitCode = await runCli(
    ["--release", "release.json", "--repo", "owner/name"],
    options
  );
...
test("release live creation with explicit head avoids current branch resolution", async () => {
  const client = new FakeGitHubClient([], [
...
test("invalid release metadata fails before branch resolution or GitHub client creation", async () => {
  ...
  const exitCode = await runCli(["--release", "release.json"], options);
...
test("release branch resolution failure asks for explicit head before creating a GitHub client", async () => {
  const stderr: string[] = [];
  const options: ManualPrRunCliOptions = {
    loadRelease: async () => ({ ok: true, value: validRelease }),
...
test("release live creation forwards pull request input and reports created PR details", async () => {
  const client = new FakeGitHubClient([], [
...
test("release live creation failure includes the underlying GitHub error", async () => {
  const client = new FakeGitHubClient([], [
    {
      ok: false,
      error: {
